package me.supercube.common.entity.search.utils;
...
import me.supercube.common.entity.search.*;
import me.supercube.common.entity.search.exception.*;
import me.supercube.common.entity.search.filter.*;
import me.supercube.common.util.SpringUtils;
...
public final class SearchableConvertUtils {
	...
	public static <T> void convertSearchValueToEntityValue(
			final Searchable search, final Class<T> entityClass) {
	...
	private static void convertSearchValueToEntityValue(
			BeanWrapperImpl beanWrapper, SearchFilter searchFilter) {
		...
		if (searchFilter instanceof OrCondition) {
			for (SearchFilter orFilter : ((OrCondition) searchFilter)
					.getOrFilters()) {
		...
		if (searchFilter instanceof AndCondition) {
			for (SearchFilter andFilter : ((AndCondition) searchFilter)
					.getAndFilters()) {
	...
	private static void convert(BeanWrapperImpl beanWrapper, Condition condition) {
		...
		if (isCollection || isArray) {
			...
			for (int i = 0; i < length; i++) {
				list.set(
						i,
						getConvertedValue(beanWrapper, searchProperty,
								entityProperty, list.get(i)));
			}
			newValue = list;
		} else {
			newValue = getConvertedValue(beanWrapper, searchProperty,
	...
	private static Object getConvertedValue(final BeanWrapperImpl beanWrapper,
			final String searchProperty, final String entityProperty,
			final Object value) {
		...
		try {
			...
			throw new InvalidSearchPropertyException(searchProperty,
					entityProperty, e);
		} catch (Exception e) {
			throw new InvalidSearchValueException(searchProperty,
					entityProperty, value, e);
		}
