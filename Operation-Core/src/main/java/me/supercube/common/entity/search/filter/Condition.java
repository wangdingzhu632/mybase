...
import me.supercube.common.entity.search.exception.InvlidSearchOperatorException;
import me.supercube.common.entity.search.exception.SearchException;
...
public final class Condition implements SearchFilter {
	 ...
	 *            值
	 */
	private Condition(final String searchProperty,
	 ...
	 * @return
	 */
	static Condition newCondition(final String key, final Object value)
		...
		if (searchs.length == 0) {
			throw new SearchException(
					"Condition key format must be : property or property_op");
		}
		...
		if (searchs.length == 1) {
			...
			try {
				...
				throw new InvlidSearchOperatorException(searchProperty,
						searchs[1]);
			}
		}
		...
		isValueBlank = isValueBlank
				|| (value instanceof String && !StringUtils.hasLength((String) value));
		isValueBlank = isValueBlank
	 ...
	 * @return
	 */
	static Condition newCondition(final String searchProperty,
			final SearchOperator operator, final Object value) {
	...
	public boolean isUnaryFilter() {
		...
		return StringUtils.hasLength(operatorStr)
				&& operatorStr.startsWith("is");
	}
	...
	public String toString() {
		return "Condition{" + "searchProperty='" + searchProperty + '\''
